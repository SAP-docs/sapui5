...
Get to know the message types supported by SAP Fiori elements for OData V4.
...
Navigation is also supported from the messages. This allows users to navigate directly to the place where the error/warning occurs on the UI by simply clicking on the respective message in the message popover.
...
-   You want to display unbound messages that provide immediate feedback to users on an operation they performed.
...
> ### Note:  
> For information about SAP Fiori elements for OData V2, see [Using Messages](using-messages-99cdc0c.md).
