...
You can extend the filter bar by using a custom filter field in SAP Fiori elements for OData V4.
...
You can set custom filters with default values by configuring the `manifest.json` file as shown in the following sample code:
...
>                             "Rating": {
>                                 "settings": {
>                                     "defaultValues": [{"operator": "EQ", "values": ["A+"]}]
>                                 }
>                             }
>                         }
>                     }
