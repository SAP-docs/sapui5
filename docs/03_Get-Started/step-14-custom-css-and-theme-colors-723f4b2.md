...
We create a folder `css`, which will contain our CSS files. In a new style definition file inside the `css` folder, we create our custom classes combined with a custom namespace class. This makes sure that the styles will only be applied to controls that are used within our app.
...
In an additional class `myCustomText`, we define a bold text and set the display to `inline-block`. This time we just define our custom class without any additional selectors. As a color value, we use the custom CSS properties provided by the theming base content to set the highlight color that is defined in the theme.
