...
You must use the AI notice to show that the page contains content generated by AI in SAP Fiori elements for OData V4.
...
-   A link if `contentFragmentName` or `ContentText` is set. These keys configure what is displayed in the related popover. The `contentFragmentName` is used to load an entire fragment, whereas the property `ContentText` displays only the text that is set in the `manifest.json` file.
