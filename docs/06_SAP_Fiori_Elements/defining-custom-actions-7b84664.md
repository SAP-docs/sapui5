<!-- loio7b846643dbaf471db1d338ee9acddf84 -->
...
<a name="loio7b846643dbaf471db1d338ee9acddf84__section_ewz_fvs_dhc"/>
...
2.  Open the JavaScript file that you created and define the custom actions, as shown in the following sample code:
...
3.  Configure the `manifest.json` file by performing the following steps:

    1.  Add your JavaScript file path in the `extends` section of the `manifest.json` file, as shown in the following sample code:
    ...
    2.  Under the `objectStreamCardsSettings` property of the stack card settings, add the `customActions` property, and define the following mandatory properties:
...
<a name="loio7b846643dbaf471db1d338ee9acddf84__section_zfy_m3p_k2b"/>

## Global Actions on the Filter Bar

To add custom global actions on the smart filter bar, you must configure the `manifest.json` file. Then, define a fragment in the view extension and create a `controller.js` file to handle the events, as shown in the following sample code:
...
Add the controller and view extensions with a path to the custom controller name and fragment name.
