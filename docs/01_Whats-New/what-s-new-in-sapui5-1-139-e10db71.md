...
# What's New in SAPUI5 1.139

With this release SAPUI5 is upgraded from version 1.138 to 1.139.
...
<sub>Changed•SAP Fiori Elements•Info Only•1.139</sub>
...
-   You can now use checkbox groups in forms. For more information, see [Grouping of Fields](../06_SAP_Fiori_Elements/grouping-of-fields-cb1748e.md).

-   You can now fill object page fields with recommended values. For more information, see [Input Assistance](../06_SAP_Fiori_Elements/input-assistance-1a6324d.md).

-   You can now navigate to any object page in a target app that has multiple entity sets and tabs. This works when the navigation context uniquely identifies a record in the target app. For more information, see [Navigation from an App \(Outbound Navigation\)](../06_SAP_Fiori_Elements/navigation-from-an-app-outbound-navigation-d782acf.md).

-   You can now configure a popin layout for responsive tables to show more information in a row. For more information, see [Setting the Table Type](../06_SAP_Fiori_Elements/setting-the-table-type-7f844f1.md).

-   Multiple actions are now sent in a single batch. For more information, see [Actions](../06_SAP_Fiori_Elements/actions-cbf16c5.md).

-   You can now set the priority of actions and group them for the overflow toolbar. For more information, see [Actions](../06_SAP_Fiori_Elements/actions-cbf16c5.md).
...
We have replaced `SmartVariantManagementUI2`, previously used for view management in the `FilterBar` control, with `SmartVariantManagement`. Please note that `SmartVariantManagementUI2` has been deprecated and is no longer recommended for use. For more information, see [Filter Bar](../10_More_About_Controls/filter-bar-2ae520a.md) and the [API Reference](https://ui5.sap.com/#/api/sap.ui.comp.smartvariants.SmartVariantManagement%23overview).
...
We’ve now enabled the support of the `UploadSetwithTable` plugin within the tree table control. Previously, the plugin was available for MDC, responsive, and grid tables. The `isDirectoryPath` property of the `rowConfiguration` aggregation must be set for this plugin to work with the tree table control. It indicates if the context of the row is a directory or a file.

For more information, see the [API Reference](https://ui5.sap.com/#/api/sap.m.plugins.UploadSetwithTable) and the [Sample](https://ui5.sap.com/#/entity/sap.m.plugins.UploadSetwithTable/sample/sap.m.sample.UploadSetwithTablePlugin.FileNameValidationSample).
...
We've introduced the `multiColumnSort` API that supports sorting for multiple columns in the gantt chart variant. This API is available at the `GanttChartWithTable` level. To use the `multiColumSort` API, a developer must pass columns listed in the order they're to be applied by using the `newValue` array, and the `oldValue` array to represent the previous sort order of every column. Developers must ensure that both the arrays follow the same order to reflect the sort priority accurately.

For more information, see the [API Reference](https://ui5.sap.com/#/api/sap.gantt.simple.GanttChartWithTable%23methods/multiColumnSort).
...
We have provided a `changeModelValue` event that is triggered whenever a similar event is fired in the `SmartField` control of the `SmartTable`. This event can be used instead of the `fieldChange` event. For more information, see the [API Reference](https://ui5.sap.com/#/api/sap.ui.comp.smarttable.SmartTable%23events/Summary) for the `changeModelValue` event.
