...
Semantic key fields in tables are displayed with a special logic. You can add the semantic key in the list report tables as shown in the following sample code:
...
-   For other tables, SAP Fiori elements uses a label and renders the entire content in bold. SAP Fiori elements also reads the associated text and combines the information in accordance with how the `TextArrangement` is configured. If no `TextArrangement` is configured, SAP Fiori elements for OData V4 displays only the text. If no text is available, we display only the ID.
...
> ### Note:  
> For information about SAP Fiori elements for OData V2, see [Handling Semantic Key Fields](handling-semantic-key-fields-94c1856.md).
