<!-- loio94c1856a672a42cbbddf3cb570b1b043 -->
...
-   For other tables, SAP Fiori elements uses a label and renders the entire content in bold. SAP Fiori elements also reads the associated text and combines the information in accordance with how the `TextArrangement` is configured. If no `TextArrangement` is configured, SAP Fiori elements displays the ID along with the text in parenthesis if text is available. If no text is available, we display only the ID.
