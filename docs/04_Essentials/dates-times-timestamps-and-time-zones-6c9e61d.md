...
> ### Restriction:  
> If you use a configured time zone, **your application may break** if it uses JavaScript `Date` objects in combination with `oDate.getHours()` or `oDate.getDate()`. To avoid such issues, we strongly recommend to use data binding with the corresponding SAPUI5 OData types wherever possible in your application.
