...
## Initial Sorting, Grouping and Filtering

The concepts described in this section apply to both list bindings and tree bindings. The examples use a list binding, but the same approach works for tree bindings.
...
In XML views, group header factory functions and group functions must be defined as controller methods and referenced using a dot \(`.`\) prefix, like in the example above `groupHeaderFactory: '.getGroupHeader'`. With this notation, the function's `this` is bound to the controller.
...
The example shown here will only display companies with a city name starting with `'B'` and a revenue of less than 150 million. You can provide more than one filter, each of which may refer to different properties using different filter operators. For a complete list of permitted filter operators, see [sap.ui.model.FilterOperator](https://ui5.sap.com/#/api/sap.ui.model.FilterOperator) in the *API Reference* part of the Demo Kit.

Initial sorting, grouping and filtering can also be provided using JavaScript.
...
], function(Sorter, Filter, FilterOperator, FilterType) {

//returns group header captions
const fnGetCounty = (oContext) => oContext.getProperty('county');
...
const oFilterCity = new Filter("city", FilterOperator.StartsWith, "B");
const oFilterRevenue = new Filter("revenue", FilterOperator.LT, 150000000);
});
```

You can pass sorters and filters to the binding:

```js
// List imported from sap/m/List
const oList = new List({
    items: {
        path: "/companies",
        template: oItemTemplate,
        sorter: oSorter,
        filters:[oFilterCity, oFilterRevenue]
    }
});
```
...
## Modifying Sorting and Filtering at Runtime

To modify the sorting or filtering after the binding has been created, retrieve the binding and call its `sort` or `filter` function:
...
// manual filtering
oList.getBinding("items").filter([oFilterCity, oFilterRevenue], FilterType.Application);
```

> ### Note:  
> `getBinding` requires the name of the bound aggregation. In this example, we are looking at the `items` aggregation of the `sap.m.List` control.
> 
> Each list binding maintains two separate sets of filters: one managed by the control itself, and one for application-defined filters. When calling `filter()`, make sure to always pass the filter type explicitly:
> 
> -   [`FilterType.Application`](https://ui5.sap.com/#/api/sap.ui.model.FilterType/overview) - for regular application filters \(static filter values\)
> 
> -   [`FilterType.ApplicationBound`](https://ui5.sap.com/#/api/sap.ui.model.FilterType/overview) - for bound application filters with values that come from data binding expressions \(see [Bound Filters](sorting-grouping-and-filtering-for-list-binding-and-tree-binding-ec79a5d.md#loioec79a5d5918f4f7f9cbc2150e66778cc__section_BF)\)
> 
> 
> If missing, the default behavior differs by model. For client-side models such as the `JSONModel`, the filters are treated as control filters and any existing application filters are cleared. For more information, see the `filter` method of the respective list or tree binding, for example [`v4.ODataListBinding#filter`](https://ui5.sap.com/#/api/sap.ui.model.odata.v4.ODataListBinding%23methods/filter) or [`ClientListBinding#filter`](https://ui5.sap.com/#/api/sap.ui.model.ClientListBinding%23methods/filter).



<a name="loioec79a5d5918f4f7f9cbc2150e66778cc__section_jxn_fmp_rcb"/>

## Using Complex Syntax to Add Filters and Sorters in an XML View

The `filters` and `sorter` properties of the aggregation binding info accept one or multiple filter and sorter objects defined using complex binding syntax. The supported properties correspond to the parameters of the [`sap.ui.model.Filter`](https://ui5.sap.com/#/api/sap.ui.model.Filter%23constructor) and [`sap.ui.model.Sorter`](https://ui5.sap.com/#/api/sap.ui.model.Sorter%23constructor) constructors.
...
-   To replace unbound application filters, use the `Application` filter type. These are filters that you initially set using the `filters` property of the [aggregation binding info](https://ui5.sap.com/#/api/sap.ui.base.ManagedObject.AggregationBindingInfo%23overview). Any existing bound filters are preserved when using this filter type.

If you want to completely replace the bound filter set with a different filter definition \(for example, switching from name and department filters to an age range filter\), call `filter()` with `FilterType.ApplicationBound`. The filter values passed here can be binding expressions, just like when declaring `boundFilters` in the XML binding syntax.

**Example**: Replace the name/department bound filters with bound age range filters
