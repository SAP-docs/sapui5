...
-   When you navigate to a list report page application using intent-based navigation with a navigation context:
...
If the source application provides all the key fields in the URL to uniquely identify an object page \(or subobject page\) record, then the link opens directly instead of the list report page or analytical list page. Otherwise, the link opens the list report page or analytical list page.
...
Applications can also trigger deep linking to object pages by only providing all semantic keys corresponding to the list report page entity set. In this case either the draft object page instance is loaded \(if a draft exists for the user\) or the active object page is loaded \(if no draft exists\).
